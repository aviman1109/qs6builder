...
REM ==================================================================
set SHORTNAME="${Ets.Install.ServiceName}"
set DISPLAYNAME="${Ets.Install.ServiceName}"
set DESCRIPTION="This service will invoke server.bat in the home directory of ${Ets.Install.ServiceName}, and has the same effect with executing server.bat in command line."
REM ========================================================
...
if /I "%IS_DOMAIN%" == "true" (
  ...
  set STARTPARAM="/c \"set NOPAUSE=Y ^^^&^^^& server.bat\""
...
%PRUNSRV% install %SHORTNAME% --DisplayName=%DISPLAYNAME% --Description %DESCRIPTION% --LogLevel=%LOGLEVEL% --LogPath="%LOGPATH%" --LogPrefix=service --StdOutput=auto --StdError=auto --StartMode=exe --StartImage=cmd.exe --StartPath="%JBOSS_HOME%\.." ++StartParams=%STARTPARAM% --StopMode=exe --StopImage=cmd.exe --StopPath="%JBOSS_HOME%\bin"  ++StopParams=%STOPPARAM%
goto cmdEnd
