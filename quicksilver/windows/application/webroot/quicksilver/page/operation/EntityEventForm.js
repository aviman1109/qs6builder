var EntityEventForm =
{
	doLoad: function()
	{
		...
		EntityEventForm.defaultContentLabel = form.getControl("FContent").getArgs().title;
		var clearFields = [
	...
	doTypeChange: function()
	{
		...
		form.setFieldEnabled("FNoticeId", isNotice, true);
		form.setFieldEnabled("FContent", !/SaveForm|Privilege|Notice|TerminateProcess/.test(type), !isWorkflow);
		UserPickList.setEnabled(isNotice);
		var label = EntityEventForm.defaultContentLabel;
...
EntityForm.validate = function()
{
	...
	if (JSON.stringify(UserPickList.getData()).length > clientData.maxNoticeRecipientsTextLength) {
		Jui.message.alert($text("Qs.Notice.RangeTextTooLong"));
		form.getTabStrip().setCurrentTabById("NoticeRange");
		return false;
	}
	return true;
