...
BirtCommunicationManager.prototype =
{
	...
	connect: function( )
	{
		...
		*/
		/*
			Chrome will give a 'Refused to set unsafe header "Connection"' error.
			according to https://bugs.eclipse.org/bugs/show_bug.cgi?id=144598, Connection header is added
			for eclipse embedded browser. In a standalone browser, it can removed.
			--Minglei
		*/
		var myAjax = new Ajax.Request( birtSoapRequest.getURL( ), { method: 'post', postBody: xmlDoc,
			contentType: 'text/xml; charset=UTF-8',
			onSuccess: this.responseHandler, onFailure: this.invalidResponseHandler,
			requestHeaders: ['SOAPAction', '""', 'request-type', 'SOAP'] } );
		//quicksilver end-------------------------------------------
